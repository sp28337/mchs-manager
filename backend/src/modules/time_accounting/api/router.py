...
from src.modules.time_accounting.infrastructure.write.adapters import PersonnelEmployeeExistence
...
# Пояс отсчёта календарных суток для инварианта 6.1.6.
#
# ОТКРЫТЫЙ ВОПРОС, требующий решения заказчика: ФПС работает в 11 часовых
# поясах, и ни один документ проекта пояс отсчёта не называет. На вердикт
# этой конкретной проверки он повлиять не может (превысить 24 ч за сутки
# способен только набор с пересечением, а пересечение от пояса не
# зависит — см. докстринг `DailyServiceTimeLimitService`), поэтому здесь
# он безопасен. Но Алгоритмы Г-Е (ночные/праздничные/выходные часы) на
# него уже завязаны по существу, и к моменту их реализации пояс обязан
# определяться местом службы — вероятнее всего колонкой `personnel.unit`.
_ACCOUNTING_TIME_ZONE = ZoneInfo("Europe/Moscow")
...
async def register_service_time_event(
    ...
    handler = RegisterServiceTimeEventHandler(
        ...
        DailyServiceTimeLimitService(time_zone=_ACCOUNTING_TIME_ZONE),
    )
    try:
...
async def approve_timesheet(
    ...
    try:
        timesheet = await handler.handle(ApproveTimesheetCommand(timesheet_id=timesheet_id))
    except TimesheetNotFoundError as exc:
        raise _problem(404, "not-found", "Табель не найден", str(exc)) from exc
    except TimesheetApprovedError as exc:
