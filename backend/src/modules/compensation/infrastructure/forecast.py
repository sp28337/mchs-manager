...
`forecast_rest_days` считается делением часов на длительность суток
отдыха. Величина взята 24 ч и вынесена константой, а НЕ в правила, и это
осознанное упрощение с известной ценой: порядок пересчёта часов
переработки в дополнительные сутки отдыха устанавливается ведомственным
актом (Приказ № 410), которого у проекта нет. Пока его нет, любая
формула здесь — предположение; выбрано самое прозрачное, чтобы ошибка
была видна, а не спрятана в правдоподобном коэффициенте.

Помечено `TODO(410)`: при появлении приказа величина переезжает в
`RuleVersion`, как и всё остальное, что установлено актом.
"""
...
# TODO(410): порядок пересчёта часов в сутки отдыха устанавливается
# ведомственным актом — см. докстринг модуля.
HOURS_PER_REST_DAY = Decimal(24)
