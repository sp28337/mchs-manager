...
`legal_base` не историзирован. Алгоритм А шаг 4 требует определять
правовую базу на дату периода, но `service_record_entry` её не хранит —
её вообще нет в логической модели как исторического факта, только как
текущий атрибут `employee`. Полноценное решение — писать `legal_base` в
летопись при переходе из гражданского персонала в аттестованный состав, и
это изменение схемы, требующее решения о том, каким событием такой переход
оформляется.

Пока правовая база на дату берётся текущей, а расчёт записывает
использованное значение в собственный провенанс
(`hours_breakdown_projection.computed_from_legal_base`) — так что
расхождение видно, а не спрятано. Это меньшее из зол: посчитанное число
объяснимо задним числом, даже если основание для него было выбрано
приблизительно.
"""
