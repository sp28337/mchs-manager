 ...
 * * цвета и буквы суток — из легенды расчёта: «О» отпуск, «Б» больничный,
 *   «РЗ» резерв, «В» свободные сутки;
...
interface Becomes {
   ...
   */
  step: number;
  /** Место внутри группы: отпуск ложится днями подряд, а не разом. */
  order?: number;
}
    ...
    <Panel>
      <div className="w-full max-w-104 space-y-5">
        <div className="relative space-y-3 rounded-xl border border-rule bg-paper-raised p-4">
          ...
          <Pointer className="demo-tap-schedule right-7 top-4" />
        </div>

        {/* Две недели на одном месте: нижняя задаёт размер, верхняя лежит
            поверх и уходит. Подменить содержимое нечем — страница отдаётся
            статикой. */}
        <div className="relative">
          <div className="demo-strip-next">
            <WeekGrid
              cells={WEEK_TWO_TWO}
              cellClassName={() => "demo-cell-in"}
...
/* ========================================================================
   3. ОТПУСКА, БОЛЬНИЧНЫЕ И ВЫЗОВЫ — на целом месяце.

   Показывать это парой суток бессмысленно: отпуск и больничный — ПЕРИОДЫ,
   и вся их суть в том, сколько смен они накрыли и что при этом стало с
   нормой. На двух клетках периода не видно вовсе.
   ...
   * больничный и отпуск уменьшают НОРМУ (письмо Роструда № 550-6-1);
   * вызов в резерв прибавляется к ОТРАБОТАННОМУ и норму не трогает
     (ст. 54 ФЗ-141, ст. 91 ТК РФ).
 ...
 * резерв одними сутками. Порядок отметок и есть порядок в череде: сперва
 * человек видит одну запись, потом другую.
 */
