...
/** Строки под записи — шесть, и все заполнены. */
...
const ROWS: readonly number[] = [245, 287, 329, 371, 413, 455];
 ...
 * Записи в табеле.
 *
 * Сутки через трое: у первого караула смены 1, 5, 9-го и так далее, у
 * второго — 2, 6, 10-го. В сутках между сменами стоит «В», как и
 ...
 *
 * Караул при этом не остаётся пустым: пятая строка — тот, кто выходит в
 * его смены, пока третий в отпуске. Бланк должен сходиться сам с собой,
 * иначе он перестаёт быть бланком.
 *
...
const GUARDS: readonly { name: string; firstShift: number | null }[] = [
  ...
  { name: "Смирнов Е. В., пожарный", firstShift: 3 },
  { name: "Морозов А. С., водитель", firstShift: 1 },
];
...
const FILL = { row: 0.14, cell: 0.011 };
...
export function TabelBackdrop() {
  return (
    <div
      ...
      //
      // По горизонтали — два разных случая.
      //
      // От `xl` документ вписан в полосу первого экрана: она сама шире
      // колонки `main`, и правый край бланка совпадает с её краем — с
      // тем же концом линии, что отделяет первый экран, и с той же
      // вертикалью, от которой начинается заголовок. Так у документа
      // появляется граница, к которой он привязан, а не случайный обрыв.
      //
      // Ниже `xl` такой полосы уже нет: место кончилось. Вписать бланк в
      // окно значило бы ужать его до нечитаемой сетки — клетка стала бы
      // вдвое мельче, чем на большом экране, и документ превратился бы в
      // серую штриховку. Поэтому там он, наоборот, ШИРЕ окна и уходит
      // вправо за край: клетка остаётся того же размера, а теряется
      // правый край бланка, где всё равно пусто. Ширины подобраны так,
      // чтобы левый край слоя стоял у левого края окна — иначе маска,
      // которая уводит документ из-под текста, начиналась бы за экраном и
      // не работала бы.
      //
      // Секция центрирована, её середина совпадает с серединой окна,
      // поэтому отсчёт от 50% выводит слой наружу, а лишнее срезает
      // `overflow-x: clip` в разметке страницы.
      className="tabel-backdrop pointer-events-none absolute top-24 bottom-14 right-[calc(50%-65vw)] z-0 hidden w-[115vw] select-none text-ink sm:block lg:right-[calc(50%-58vw)] lg:w-[104vw] xl:right-0 xl:w-[92%]"
    >
      <svg
