 ...
 * --- Из чего он сделан ---------------------------------------------------
 *
 * Холст — почти максимальный чёрный рамы пожарного автомобиля, панели на
 * несколько ступеней светлее, линовка взята из резины: она серее краски и
 * потому видна на кузове сама по себе. Панель — не наклейка на холсте, а
 * его же поверхность, поднятая на пару процентов прозрачной заливкой:
 * свечение проходит сквозь неё, и блок принадлежит фону. Держит панель
 * волосяная рамка, а не тень; на наведении рамка набирает синий.
 *
 * Синий — проблесковый маячок, и он же цвет продукта: свечение,
 * надзаголовки, грань панели. Бирюза `verify` осталась тем, чем была, —
 * меткой ПОДТВЕРЖДЁННОГО, и появляется только там, где что-то сошлось.
 * Сигнальный красный тоже на месте: он там, где требуется решение
...
const EYEBROW =
  "font-display text-[10px] font-bold uppercase leading-none tracking-[0.14em] text-beacon";
 ...
 * остального. `beacon` — единственный тон без собственного смысла: он для
 * акцента на витрине, где помечать нечего, а выделить нужно.
 */
export function Pill({
  ...
  children: ReactNode;
  tone?: "plain" | "verify" | "signal" | "trace" | "accent" | "beacon";
  className?: string;
}) {
  return (
    <span
      className={cn(
        ...
        tone === "beacon" && "border-beacon/35 bg-beacon-soft text-beacon",
