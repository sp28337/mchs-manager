...
/**
 * Чем сервис называет себя поиску.
 *
 * --- Почему не «сутки через трое» -------------------------------------------
 *
 * Приложение начиналось калькулятором для одного графика — отсюда имя и
 * домен, — но считает оно любой сменный: сутки через трое, два через два,
 * сутки через четверо, пятидневку и свой цикл. Заголовок, обещающий один
 * график, отсекал бы из выдачи всех остальных; а пришедшему по нему всё
 * равно отвечало бы то же самое приложение.
 *
 * Сами графики названы в описании, а не в заголовке: в заголовке место
 * одному ответу на вопрос «что это», а перечисление — уже подробность.
 * Названы они и на самой странице, в первом шаге раздела «Как работает», —
 * иначе слова были бы только в служебной строке, а поиск ищет по тексту.
 */
const TITLE = "Норма и переработка при сменном графике";
const DESCRIPTION =
  "Сменный график на год: сутки через трое, два через два, сутки через " +
  "четверо, 5|2 или свой цикл. Смены, отпуска, больничные, работа помимо " +
  "графика и заметки к дню. Норма считается по производственному календарю, " +
...
export const metadata: Metadata = {
  ...
  // Ключевые слова поисковики давно не взвешивают, и держатся они здесь не
  // ради веса, а как перечень запросов, на которые страница обязана
  // отвечать: по нему сверяется, названы ли эти слова в тексте. Порядок —
  // от общего к частному, а не от частого к редкому.
  keywords: [
...
const FAQ: { question: string; answer: string }[] = [
  ...
  {
    question: "Какой учётный период применяется при сменном графике?",
    answer:
