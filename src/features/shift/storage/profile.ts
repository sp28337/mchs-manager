...
export const storedProfileSchema = z.object({
  ...
  /**
   * Своя недельная норма, строкой, или `null` — норма по основанию из
   * закона.
   *
   * Заведена затем, что оснований сокращённой недели больше трёх: её ставят
   * коллективным договором, отраслевым соглашением, приказом по части, и
   * бывает она какой угодно. Перечислить всё списком нельзя, а человек с
   * приказом на руках свою норму знает точно (`deriveWeeklyNorm`).
   *
   * Строкой — по тому же доводу, что и остальные часы: 39,5 при первом же
   * круге записи и чтения в JSON превратилось бы в 39.499999999999996.
   *
   * Необязательное с умолчанием `null`: профили, сохранённые до появления
   * поля, читаются как есть, и норма у них остаётся прежней — той, что
   * выводится из условий труда и инвалидности.
   */
  weeklyNormHours: z.string().max(6).nullable().default(null),
   ...
   *
   * Имя ключа осталось от времён, когда мера звалась «сутками» (на экране
   * это теперь «Переработка в сменах»). Переименовать его значило бы
   * завести разбор старого имени ради нового слова — у людей на
   * устройствах и в выгруженных файлах лежит именно `overtimeInDays`.
   */
...
export function createProfile(input: NewProfileInput): StoredProfile {
  return {
    ...
    countFrom: null,
    // Своей нормы у нового профиля нет: анкета спрашивает основание из
    // закона, а «своя» — выход для тех, у кого приказ, и ставят её потом,
    // в настройках.
