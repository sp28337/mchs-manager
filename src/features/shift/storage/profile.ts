 ...
 * безымянная страница выглядит недоделанной. Одно и то же слово при
 * заведении профиля и при сбросе настроек.
 */
export const DEFAULT_PROFILE_NAME = "Мой график";

/**
 * Настройки по умолчанию — данные при этом остаются.
 *
 * --- Что сбрасывается и что нет -------------------------------------------
 *
 * Сбрасываются ОТВЕТЫ О СЕБЕ и способ показа: имя, основание недельной
 * нормы, время отсчёта смены, режим «Онлайн», мера переработки. Это то, в
 * чём человек мог ошибиться при заведении профиля и что можно назвать
 * заново за полминуты.
 *
 * НЕ сбрасывается ничего, чего приложение не сможет восстановить: дата
 * рабочей смены, отпуска и больничные, вызовы, правки производственного
 * календаря, переносы смен, заметки. Дата смены здесь не придирка: она
 * задаёт весь график, и «сбросить» её можно только выдумав другую — то
 * есть молча построив человеку чужой график вместо его собственного.
 *
 * Для «стереть всё» есть отдельный, честно названный способ: удалить
 * профиль с устройства.
 */
export function resetSettings(profile: StoredProfile): StoredProfile {
  return {
    ...profile,
