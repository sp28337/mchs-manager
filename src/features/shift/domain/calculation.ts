 ...
 * --- Чего здесь нет -----------------------------------------------------
 *
 * Компенсаций и доплат. При суммированном учёте ночные, выходные и
 * праздничные часы В ПРЕДЕЛАХ нормы сами по себе ничего сверх не дают:
 * чем и как они оплачиваются, решают трудовой договор и правила
 * внутреннего трудового распорядка, а они у всех разные. Показывать их
 * как «положено сверху» значило бы обещать то, чего расчёт не знает.
 * Ночные часы считаются и показываются — но как факт, а не как основание
 * для доплаты.
 */
 ...
 * начала смены. Число вынесено сюда, потому что по нему же строится множество
...
export function calculatePeriod({
  ...
  // накануне, отдаёт периоду свой хвост (с полуночи до её начала). Начинать
  ...
  // (ст. 91 ТК РФ): часы идут в ОТРАБОТАННОЕ и норму не трогают. Ночные по
  // ним не считаются — распоряжение о вызове задаёт число часов, а не
