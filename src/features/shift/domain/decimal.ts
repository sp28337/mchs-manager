...
/**
 * Форма слова при числе: 1 смена, 2 смены, 8 смен.
 *
 * --- Зачем общее правило ---------------------------------------------------
 *
 * Слов, которые приложение ставит рядом с числом, набралось много: смены,
 * правки, рабочие и праздничные дни в подписи месяца. Правило у всех одно и
 * то же, и написанное заново оно каждый раз спотыкается об одиннадцать:
 * «21 рабочий», но «11 рабочих» — последняя цифра там обманывает. Так и
 * появлялись подписи вроде «1 праздничных».
 *
 * Три формы, а не две: русскому числительному их и нужно три — при одном,
 * при двух-четырёх и при остальных.
 */
export function numberWord(count: number, one: string, few: string, many: string): string {
