 ...
 *
 * --- Почему хранится промежутком, а спрашивается иначе ---------------------
 *
 * Спрашивают у человека начало и продолжительность — теми же двумя
 * величинами, что стоят в настройках, и в том же порядке. Это его язык:
 * «заступил в восемь, отработал двенадцать». Конца смены он не набирает
 * вовсе — тот и так следует из двух названных чисел, а лишний ответ,
 * который можно вычислить, это лишний способ ошибиться.
 *
 * Хранится же и считается ПРОМЕЖУТОК. Расчёту нужен именно конец: он
 * раскладывает смену по двум календарным суткам, и от того, куда попал
 * каждый кусок, зависят ночные часы (ст. 96 ТК РФ) и то, в каком месяце
 * эти часы окажутся. Перевод из одного вида в другой лежит рядом
 * (`spanFrom`) и делается ровно в одном месте.
 */
...
/**
 * Часы смены из начала и продолжительности.
 *
 * Человек называет смену именно так — «заступил в восемь, отработал
 * двенадцать», — а хранится она промежутком: конец нужен расчёту, чтобы
 * разложить смену по двум суткам, и он же отвечает на вопрос «когда
 * сдавать». Перевод собран здесь один раз, потому что нужен и графику, и
 * отдельным суткам.
 */
export function spanFrom(startsAt: string, durationMinutes: number): ShiftSpan {
