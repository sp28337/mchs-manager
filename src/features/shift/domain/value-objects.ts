...
export const ABSENCE_KIND_BASIS: Record<AbsenceKind, string> = {
  ...
  sick_leave: "ст. 183 ТК РФ",
  study_leave: "ст. 173-177 ТК РФ",
  time_off_in_lieu: "ст. 152, 153 ТК РФ",
};
 ...
 * ОТРАБОТАННОЕ сверх нормы (ст. 152 ТК РФ), то есть
 ...
 *
 * Всё перечисленное — исполнение трудовых обязанностей, то есть рабочее
 * время (ст. 91 ТК РФ): часы прибавляются к отработанному. Виды
 ...
 *
 * `callout` — вызов без уточнения, и он здесь первым намеренно. Названные
 * виды закрывают не всё: человека вызывают и просто так — подменить, помочь,
 * доделать, — и до сих пор такой выход приходилось записывать чужим видом,
 * лишь бы часы попали в расчёт. Разбор с руководителем от этого становился
 * разговором о «резерве», которого не было.
 *
 * Порядок в записи — это и порядок в списке и легенде: общий случай стоит
 * первым, частные за ним.
 */
...
export const CALLOUT_KIND_BASIS: Record<CalloutKind, string> = {
  callout: "ст. 91 ТК РФ",
  competition: "ст. 91 ТК РФ",
  training_camp: "ст. 91 ТК РФ",
  reserve: "ст. 91 ТК РФ",
  public_event: "ст. 91 ТК РФ",
  elections: "ст. 91 ТК РФ",
};
