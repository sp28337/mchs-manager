...
/**
 * Сегодняшняя дата — по календарю ТОГО, КТО СМОТРИТ, а не по Гринвичу.
 *
 * Здесь стояло `toISOString().slice(0, 10)`, и это давало дату в UTC.
 * Ошибка не теоретическая: в Москве (UTC+3) с полуночи до трёх часов ночи
 * «сегодня» возвращалось вчерашним, а на Камчатке (UTC+12) — целую
 * половину суток, с полуночи до полудня. Приложением пользуются в смену,
 * в том числе ночью и в том числе за Уралом, — то есть ровно тогда и там,
 * где промах случался.
 *
 * Цена промаха: режим «Онлайн» обрезал расчёт по вчерашний день, а сегодня
 * на сетке было бы отмечено не тем числом — при том, что отметка и нужна
 * человеку затем, чтобы найти себя в году.
 *
 * `getFullYear`/`getMonth`/`getDate` читают именно местный календарь, и
 * часовой пояс в ответ не попадает вовсе — как и во всей остальной
 * арифметике этого файла.
 */
