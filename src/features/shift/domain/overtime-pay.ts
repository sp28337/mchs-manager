 ...
 * деньги — приказ МЧС России от 21.03.2013 № 195 для сотрудников и приказ
 ...
 *
 * СОТРУДНИК (приказ № 195). Часовая ставка задана приказом жёстко:
 * должностной оклад, делённый на среднемесячное количество рабочих часов
 * по производственному календарю на год (п. 95). Порог полуторного
 * размера при суммированном учёте тоже задан — в среднем два часа на
 * каждый рабочий день учётного периода (п. 97).
 *
 ...
 * № 195 — 20 % часовой ставки; п. 19 приказа № 747 — 35 % и выше),
...
/** Приказ № 195 п. 94(а); ч. 1 ст. 152 ТК РФ. */
const FIRST_HOURS_MULTIPLIER = new Dec("1.5");
const LATER_HOURS_MULTIPLIER = new Dec(2);

/**
 * Приказ № 195 п. 97: полуторный размер — за часы, «не превышающие в
...
export const PAY_BASIS: Record<EmploymentKind, string> = {
  attested: "Приказ МЧС России от 21.03.2013 № 195, пп. 94, 95, 97",
  civilian:
...
export interface OvertimePayInput {
   ...
   *
   * Именно года, а не учётного периода: п. 95 приказа № 195 говорит
 ...
 *
 * Для сотрудника прочтение одно: п. 97 приказа № 195 не оставляет выбора.
 ...
 * треть от месяца к месяцу. Приказ № 195 п. 95 требует именно годовую
 ...
 * --- Про отгулы ----------------------------------------------------------
 *
 * Пункт 99 приказа № 195 требует не включать в оплачиваемые часы то, за
...
export function calculateOvertimePay(input: OvertimePayInput): OvertimePayEstimate | null {
  ...
  // приказа № 195 не оставляет для него места, и предлагать выбор там,
