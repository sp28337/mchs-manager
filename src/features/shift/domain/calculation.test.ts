...
describe("полный расчёт", () => {
  ...
  test("смена через границу суток делит свои часы", () => {
    // При начале смены в 08:30 смена с 30 марта отдаёт 30-му 15,5 часа, а 31-му
    // 8,5. Время начала задано здесь явно, а не взято из умолчания:
...
describe("отгул", () => {
  test("норму не уменьшает, а гасит переработку", () => {
    // Отгул даётся ЗА УЖЕ ОТРАБОТАННОЕ сверх нормы (ст. 152 ТК РФ).
    // Уменьшить за него ещё и норму значило бы заплатить
