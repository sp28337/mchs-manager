...
describe("ввод и вывод сумм", () => {
  ...
  /**
   * Поле оклада разбирается на каждое нажатие клавиши, поэтому «не число»
   * там — не редкость, а обычное промежуточное состояние: человек стёр
   * всё и набирает заново, задел букву, вставил сумму из расчётного
   * листка вместе со словом «руб».
   *
   * `decimal.js` сообщает о непригодной строке ИСКЛЮЧЕНИЕМ, а не
   * значением, и одна буква в поле роняла весь экран расчёта. Разбор
   * обязан возвращать `null`, а не бросать.
   */
  it("буквы и мусор отвергаются, а не роняют расчёт", () => {
    for (const input of ["и", "i", "30000 руб", "--", "1,2,3", "٣٠", "0x", "e5"]) {
      expect(() => parseMoney(input)).not.toThrow();
      expect(parseMoney(input)).toBeNull();
    }
  });

  it("«Infinity» и «NaN» — тоже не суммы", () => {
    expect(parseMoney("Infinity")).toBeNull();
    expect(parseMoney("NaN")).toBeNull();
  });

  it("неразрывный пробел разделителем разрядов разбирается", () => {
    // Ровно то, что кладётся в буфер при копировании суммы с этой же
    // страницы: `formatMoneyAmount` разделяет разряды U+00A0.
    expect(parseMoney("48\u00a0000")?.toString()).toBe("48000");
    // И узкий неразрывный U+202F: именно его ICU отдаёт для ru-RU в
    // новых версиях Node.
    expect(parseMoney("48\u202f000")?.toString()).toBe("48000");
  });
});
