 ...
 * августе их не бывает, и «0 пра» занимало бы место, ничего не сообщая.
 */
function MonthMeta({ facts, edited }: { facts: MonthFacts; edited: number }) {
  return (
    <>
      <CountedNumber value={String(facts.working)} /> раб
      {facts.holidays > 0 ? (
        <span className="text-signal">
          {" / "}
          <CountedNumber value={String(facts.holidays)} /> пра
        </span>
      ) : null}
