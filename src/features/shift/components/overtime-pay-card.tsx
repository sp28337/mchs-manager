 ...
 *
 * --- Три оговорки, без которых число вводит в заблуждение ---------------
 *
 * Они не мелким шрифтом внизу «на всякий случай», а рядом с суммой,
 * потому что каждая меняет ожидание:
 *
 * 1. Деньги не начисляются сами — нужен рапорт (п. 94 приказа № 195).
 * 2. Ставка считается от ДОЛЖНОСТНОГО ОКЛАДА, а не от денежного
 *    довольствия: надбавки за звание, выслугу и особые условия в неё не
 *    входят (п. 95). Сумма выйдет меньше, чем человек ожидает, и не
 *    сказать об этом — значит получить обвинение в ошибке.
 * 3. Это начислено, до НДФЛ.
 */
        ...
        <p className="max-w-prose text-xs text-ink-muted">
          {attested ? (
            <>
              ...
              ставку не входят (Приказ МЧС России от 21.03.2013 № 195, п. 95).
            </>
          ) : (
            <>
      ...
      {pay === null ? null : (
        <div className="space-y-4 border-t border-rule pt-4">
          ...
          <ul className="max-w-prose space-y-1.5 text-xs text-ink-muted">
            ...
            {attested ? (
              <li>
                Компенсация выплачивается <strong>по рапорту</strong>, сама она
                не начисляется (п. 94 приказа № 195). Вместо денег можно взять
                дни отдыха (п. 100), но тогда эти часы не оплачиваются (п. 99).
              </li>
            ) : (
              <li>
    ...
    <div className="space-y-3">
      ...
      <p className="max-w-prose text-xs text-ink-muted">
        ...
        {attested ? (
          ...
          <>
            Приказ № 747 порога не устанавливает — он взят из п. 97 приказа МЧС
            России от 21.03.2013 № 195, применённого по аналогии. Размеры
