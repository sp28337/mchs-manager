 ...
 * ли это график, который висит на работе.
 *
...
export function SettingsPanel({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* «Сбросить календарь» и «Удалить профиль» стоят не в самой анкете.
          -------------------------------------------------------------
          Сброс стирает ВНЕСЁННОЕ, а не ответы анкеты, и место ему там, где
          внесённое видно: в закладке изменений, под самим перечнем.
          Удаление уносит профиль целиком и стоит в конце этой закладки,
          но ПОСЛЕ анкеты — не среди её полей.
 ...
 * вообще всё. Вид у них поэтому один: та же приглушённая кнопка со знаком,
 * то же окно с вопросом, тот же порядок слов в нём.
 *
 * --- Почему стоят они всё-таки врозь ---------------------------------------
 *
 * Каждое стоит рядом с тем, что стирает. Сброс — под перечнем внесённого:
 * там видно, что именно исчезнет. Удаление — в закладке настроек, под
 * анкетой, которую оно и уносит вместе со всем остальным.
 *
 * Стояли они и рядом, обе под перечнем правок, — и получалось, что кнопка
 * обещает убрать правки, а убирает анкету. Показывается каждая своим
 * доводом: сброс — `showReset`, удаление — наличием `onForget`.
 *
