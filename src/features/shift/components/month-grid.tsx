 ...
 * Скос вогнутого угла.
 *
 * Квадрат со стороной в радиус скругления, залитый цветом обычного дня: он
 * достраивает блок до того угла, которого в сетке нет. Лишнее у него не
 * закрашивается, а ВЫРЕЗАЕТСЯ маской — дугой из центра в противоположном
 * углу, там же, где сходятся клетки. От квадрата остаётся дужка у самого
 * угла, всё прочее прозрачно.
 *
 * --- Почему вырезается, а не закрашивается --------------------------------
 *
 * Закрашивалось: поверх квадрата лежал второй, цвета страницы, со
 * скруглением. Пока страница была ровно залита, цвета `paper` для этого
 * хватало. Под лампой она уже не ровная — к верху экрана бумага набирает
 * тёплый налёт, — и подделка перестала совпадать с подлинником: замером
 * 31 против 33-34, с РЕЗКОЙ прямоугольной границей ровно там, где скос и
 * заведён, чтобы угла не было. Два-три уровня на плавной поверхности глаз
 * не заметит, а два-три уровня вдоль прямого угла — сразу.
 *
 * Вырезанный скос ничего не подделывает: сквозь него видно ту самую
 * страницу, что и вокруг, какой бы она ни стала.
 *
  ...
  return (
    <span
      ...
      className={cn(
        ...
        corner === "br" ? "fillet-br bottom-0 right-0" : "fillet-tl left-0 top-0",
      )}
    />
  );
}
