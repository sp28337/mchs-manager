 ...
 *
 * Видов вызова в данных шесть (вызов, соревнования, сбор, резерв,
 * праздник, выборы), и у каждого был свой двухбуквенный код: «СР», «СБ»,
 * «РЗ». Человеку это стоило шести строк легенды и шести кодов, которые
 * надо помнить, а расчёту не давало ничего: все шесть считаются
 * одинаково — работа сверх графика по тем же правилам.
 *
 * Поэтому в клетке теперь один код. Отмечают тоже один вид — «Вызов»; в
 * старых профилях остальные пять сохранены как есть (схема их принимает
 * и будет принимать), и различает их подпись клетки и перечень внесённых
 * изменений, где для названия есть место целой строкой.
 *
 * «ВЗ», а не «В»: одна буква «В» занята выходным (`DAY_OFF_MARK`) и
 * отгулом в табельных кодах, и три разные вещи под одним знаком на одной
 * сетке — худшее, что можно сделать с разметкой, которую читают бегло.
 */
export const CALLOUT_MARK = "ВЗ";
