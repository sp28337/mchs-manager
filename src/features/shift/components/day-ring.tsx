  ...
  return (
    ...
    >
      {/* Страница гаснет и размывается, кроме самого дня, и погашенное
          ...
          Затемнение держится всё время, пока открыт день, — и под окном
          срока или заметки тоже. Прежде оно там снималось, а окно гасило
          страницу своим, общим для всего приложения слоем: тот светлее и
          без размытия, и в миг открытия окна страница под ним СВЕТЛЕЛА и
          обретала резкость. Выходило, что окно встаёт не поверх кольца, а
          вместо него, на другой странице. Теперь наоборот: окно это своего
      ...
      <SpanModal
        ...
        // Страница под ним уже погашена кольцом — гасить второй раз нечего.
        className="modal-over-ring"
        open={asking !== null}
        title={face?.label ?? ""}
        startsOn={day}
          ...
          {faces.map((face, index) => (
            <li
              ...
              className={cn(
                ...
                // Вполголоса — и так весь перечень, всегда. В полный голос
                // — только строка, на чей квадрат сейчас смотрят.
                // -------------------------------------------------------
                // Соседние строки при наведении гасли ещё на шаг, и
                // получалось, что подсказка на наведение ТЕМНЕЕТ: рука
                // идёт к квадрату, а словарь уходит в тень, хотя читать
                // его в этот миг и собираются. Разницы между «горит» и
                // «не горит» хватает и так, а остальные строки остаются
                // такими же, какими были до наведения.
                index === hovered ? "opacity-100" : "opacity-60",
              )}
            >
              <span
                className={cn(
