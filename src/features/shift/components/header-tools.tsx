...
/**
 * Настройки, деньги и выгрузка — три действия рабочего экрана.
 *
 ...
 * в панели над сеткой, слева, вместе с выгрузкой в файл: видно всегда,
 * не занимает ни строчки высоты.
 *
 ...
 *
 * --- Почему на узком экране это одна кнопка ------------------------------
 *
 * Три плашки с подписями в узкую строку не встают, а без подписей это три
 ...
 *
 * Порог — `xl`, и назначен он замером. Кнопки стоят не в шапке, а в
 * панели управления сеткой, слева от переключателя вида: шапку целиком
 * заняли числа итога, а эти три действия нужны реже любого из них. В
 * панели с ними соседствуют переключатель вида (370 точек) и масштаб
 * (190); три плашки с подписями — ещё 433, и втроём они встают в одну
 * строку начиная с 1280. Ниже меню появляется ВМЕСТО всего ряда, а не
 * рядом с ним.
 */
  ...
  return (
    <>
      <div
        ...
        className={cn("hidden items-center gap-2 xl:flex", className)}
      >
        {TOOL_ORDER.map((id) => {
      ...
      <ToolsMenu open={menu} onOpen={setMenu} onPick={run} className="xl:hidden" />
