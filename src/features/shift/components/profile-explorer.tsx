...
export function ProfileExplorer({
  ...
  return (
    ...
    <div className="flex flex-col gap-4">
      ...
      {/* Столбец шириной по самой длинной строке (`max-content`), а не
          растянутый на экран. В строке остались ручка, имя и две кнопки —
          на мониторе они занимали треть её ширины, а две трети оставались
          пустой бумагой; разложенные же по двум-трём колонкам, строки
          читались плитками, хотя это список.

          `minmax(0, max-content)`: шире содержимого столбец не станет, но
          и за край не выйдет — длинное имя упрётся в ширину страницы и
          обрежется многоточием, как и прежде. */}
      <ul
        className={cn(
          "grid grid-cols-[minmax(0,max-content)] gap-3 empty:hidden",
